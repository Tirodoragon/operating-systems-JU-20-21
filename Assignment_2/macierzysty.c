	...
	for(i = 0; i < 3; i++) {
		...
		switch(pid) {
			...
			case 0:							/* proces potomny */
				...
				char* args[] = {"./potomny.x", (char *) NULL}; 	/* tablica na potrzeby execv */
				if(execv(args[0], args) == -1) {		/* uruchomienie programu odpowiedzialnego za komunikaty procesow potomnych */
					printf("Numer bledu: %d\n", errno);	/* obsluga bledow execv() */
					perror("execv error:");
					_exit(EXIT_FAILURE);
				}
				break;
