...
void *wykluczanie(void *arg) {
	...
	int pthread_mutex_unlock_return;
	for(int k = 0; k < LICZBA_SEKCJI_KRYTYCZNYCH; k++) {
		if((pthread_mutex_lock_return = pthread_mutex_lock(&mutex)) != 0) {	/* obsluga bledow pthread_mutex_lock() */
			printf("Numer bledu:%d\n", pthread_mutex_lock_return);
			strerror_r(pthread_mutex_lock_return, buf, buf_max);
		...
		if((pthread_mutex_unlock_return = pthread_mutex_unlock(&mutex)) != 0) {	/* obsluga bledow pthread_mutex_unlock() */
			printf("Numer bledu: %d\n", pthread_mutex_unlock_return);
			strerror_r(pthread_mutex_unlock_return, buf, buf_max);
...
int main() {
	...
	int pthread_mutex_destroy_return;
	if((pthread_mutex_destroy_return = pthread_mutex_destroy(&mutex)) != error_0) {	/* obsluga bledow pthread_mutex_destroy() */
		printf("Numer bledu: %d\n", pthread_mutex_destroy_return);
		strerror_r(pthread_mutex_destroy_return, buf, buf_max);
